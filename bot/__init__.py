...
qb_listener_lock = qb_listener_lock = Lock()
status_reply_dict = {}
download_dict = {}
rss_dict = {}
...
# The problematic line 'srun(["cp", ".netrc", "/root/.netrc"])' has been removed.
# The 'chmod +x aria.sh' is handled by the Dockerfile, so it's removed here too.
...
# Increased sleep duration to allow services more time to initialize
sleep(5)
...
def aria2c_init():
    try:
        log_info("Initializing Aria2c (test download)")
        link = "https://linuxmint.com/torrents/lmde-5-cinnamon-64bit.iso.torrent"
        dire = DOWNLOAD_DIR.rstrip("/")
        aria2.add_uris([link], {"dir": dire})
        downloads = aria2.get_downloads()
        sleep(10) # Give more time for the test download to potentially start
        aria2.remove(downloads, force=True, files=True, clean=True)
        log_info("Aria2c test download successful.")
    except Exception as e:
